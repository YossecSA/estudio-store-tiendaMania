...
function createCard(product) {
    ...
    <div>
        ...
        <i class='bx bx-heart i_favorito' onclick="addToFavorites('${product.id}')"></i>
    </div>
    `;
}

document.addEventListener('DOMContentLoaded', function () {
    const favoriteIcons = document.querySelectorAll('.i_favorito');
...
function addToFavorites(productId) {
    ...
    const favoritoExiste = favorites.some(p => p.id === productId); //some() devuelve true si al menos un elemento del array cumple con la condición
    if (favoritoExiste) {
        Swal.fire({
            icon: 'info',
